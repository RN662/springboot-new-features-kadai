...
public class HouseController {
	...
	public String show(@PathVariable(name = "id") Integer id, Model model, @AuthenticationPrincipal UserDetailsImpl userDetailsImpl) {
		...
		List<Review> recentReviews = reviewRepository.findTop6ByHouseIdOrderByIdDesc(id);
		...
		// お気に入り済みチェック（ログイン中ユーザーのみ）
		boolean isFavorite = false;
		if (loginUser != null) {
			isFavorite = favoriteRepository.existsByUserAndHouseId(loginUser, id);
		}
